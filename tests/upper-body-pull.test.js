...
const FILE_PATH = path.join(ROOT_DIR, 'upper_body_pull.html');
...
describe('upper_body_pull.html', () => {
  ...
  describe('document head', () => {
    ...
    test('has the expected page title', () => {
      expect(document.title).toBe('Upper Body Pull Day');
    });
  ...
  describe('hero section', () => {
    test('renders the expected heading and pull-focused summary', () => {
      const h1 = document.querySelector('.hero h1');
      expect(h1).not.toBeNull();
      expect(h1.textContent.trim()).toBe('Monday • Upper Body Pull Workout');

      const summary = document.querySelector('.hero p');
      expect(summary.textContent).toMatch(/Pull-focused upper-body session covering back, biceps and core/);
    });
  });
  ...
  describe('WOD summary table', () => {
    ...
    test('lists exactly four pull exercises', () => {
    ...
    test('rows contain exercise name, sets x reps and rest time in the expected columns', () => {
      ...
      expect(lastCells[1].textContent.trim()).toBe('Dumbbell Curl');
    ...
    test('exercise names match the expected pull-day list in order', () => {
  ...
  describe('main WOD exercise cards (research pictures + instructions)', () => {
    ...
    test('Bird Dog card explicitly notes it stays bodyweight (avoids overlap with the Friday RDL session)', () => {
      const birdDogCard = cards.find((c) => c.querySelector('h3').textContent.trim() === 'Bird Dog');
      expect(birdDogCard.querySelector('.note').textContent).toMatch(/bodyweight/i);
      expect(birdDogCard.querySelector('.note').textContent).toMatch(/RDL/);
    });
  });
  ...
  describe('WOD summary/card data consistency (regression)', () => {
    test('summary table exercise names exactly match the main WOD card titles, in the same order', () => {
      ...
      // The table uses "Chest-Supported DB Row" while the card uses the
      // unabbreviated "Chest-Supported Dumbbell Row" - normalize that one
      // known abbreviation before comparing, so the test still catches any
      // *other* drift between the two data sources.
      const normalize = (name) => name.replace('Chest-Supported DB Row', 'Chest-Supported Dumbbell Row');
      expect(tableNames.map(normalize)).toEqual(cardNames);
    });
  });
